...
class LearnHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            r = yield self.client.fetch(self.courseListUrl, method='POST', headers=self.cookieHeader,
                                        body=urlencode(postData))
        except HTTPError as e:
            pass

        # get username, sidList and start play
        r, _ = yield [self.client.fetch(self.courseUrl + str(courseID), headers=self.cookieHeader),
                      self.client.fetch(self.playUrl + str(courseID), headers=self.cookieHeader)]
        d = PyQuery(r.body.decode('utf-8', 'ignore'))
        ...
        for sid in sidList:
            ...
            r = yield self.client.fetch(self.progressUrl, method='POST', headers=self.cookieHeader,
                                        body=urlencode(postData))
